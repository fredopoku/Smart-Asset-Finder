...
<ul class="sidebar-nav" id="sidebar-nav">
  ...
  <li class="nav-item">
    <a class="nav-link <?= $page != 'inquiries' ? 'collapsed' : '' ?> nav-users" href="<?= base_url."admin?page=inquiries" ?>">
      ...
      <span>Messages</span>
      <?php $message = $conn->query("SELECT COUNT(*) c FROM inquiry_list WHERE status=0")->fetch_assoc()['c']; ?>
      <?php if($message > 0): ?>
        <span class="badge rounded-pill bg-danger text-light ms-2"><?= format_num($message) ?></span>
      <?php endif; ?>
    </a>
  </li>
  <li class="nav-item">
    <a class="nav-link <?= $page != 'qr_scans' ? 'collapsed' : '' ?>" href="<?= base_url."admin?page=qr_scans" ?>">
      <i class="bi bi-qr-code-scan"></i>
      <span>QR Scan Map</span>
      <?php $qrs_count = $conn->query("SELECT COUNT(*) c FROM qr_scans WHERE DATE(scanned_at)=CURDATE()")->fetch_assoc()['c'];
            if($qrs_count > 0): ?>
        <span class="badge rounded-pill bg-info text-dark ms-2"><?= format_num($qrs_count) ?></span>
      <?php endif; ?>
    </a>
  </li>
  <li class="nav-item">
    <a class="nav-link <?= $page != 'orders' ? 'collapsed' : '' ?>" href="<?= base_url."admin?page=orders" ?>">
      <i class="bi bi-bag-check"></i>
      <span>Shop Orders</span>
      <?php $pending_orders = $conn->query("SELECT COUNT(*) c FROM orders WHERE order_status='pending'")->fetch_assoc()['c'];
            if($pending_orders > 0): ?>
        <span class="badge rounded-pill bg-warning text-dark ms-2"><?= format_num($pending_orders) ?></span>
      <?php endif; ?>
    </a>
  </li>
  <li class="nav-item">
    <a class="nav-link <?= $page != 'claims' ? 'collapsed' : '' ?>" href="<?= base_url."admin?page=claims" ?>">
      ...
      <span>Claims</span>
      <?php $pending_claims = $conn->query("SELECT COUNT(*) c FROM item_claims WHERE status=0")->fetch_assoc()['c']; ?>
      <?php if($pending_claims > 0): ?>
        <span class="badge rounded-pill bg-warning text-dark ms-2"><?= format_num($pending_claims) ?></span>
      <?php endif; ?>
    </a>
  </li>
  ...
  <li class="nav-item">
    <a class="nav-link <?= $page != 'registered_users' ? 'collapsed' : '' ?>" href="<?= base_url."admin?page=registered_users" ?>">
      <i class="bi bi-person-lines-fill"></i>
      <span>Registered Users</span>
      <?php
        $ru_count = $conn->query("SELECT COUNT(*) c FROM registered_users WHERE status=1")->fetch_assoc()['c'];
        if($ru_count > 0): ?>
        <span class="badge rounded-pill bg-primary-subtle text-primary ms-2"><?= format_num($ru_count) ?></span>
      <?php endif; ?>
    </a>
  </li>
  <li class="nav-item">
    <a class="nav-link <?= $page != 'broadcast' ? 'collapsed' : '' ?>" href="<?= base_url."admin/?page=broadcast" ?>">
  ...
  <li class="nav-item">
    <a class="nav-link <?= $page != 'system_info/payment_settings' ? 'collapsed' : '' ?> nav-system_info" href="<?= base_url."admin?page=system_info/payment_settings" ?>">
  ...
  <li class="nav-item">
    <a class="nav-link <?= $page != 'system_info/change_password' ? 'collapsed' : '' ?> nav-system_info" href="<?= base_url."admin?page=system_info/change_password" ?>">
