        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$_list_uid = (int)$_settings->userdata('id');
					$list_stmt = $conn->prepare("SELECT *, CONCAT(firstname,' ',COALESCE(CONCAT(middlename,' '),''),lastname) as `name` FROM users WHERE id!=? ORDER BY firstname,lastname ASC");
					$list_stmt->bind_param('i', $_list_uid);
					$list_stmt->execute();
					$qry = $list_stmt->get_result();
